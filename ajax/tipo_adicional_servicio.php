...
$tipo_servicio_adicional=new Tipo_adicional_servicio();
...
switch ($_GET["op"]) {
	case 'guardaryeditar':
	if (empty($id)) {
		$rspta=$tipo_servicio_adicional->insertar($descripcion,$estado,$id_usuario_modifica,$fecha_modifica);
		echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar los datos";
	}else{
         $rspta=$tipo_servicio_adicional->editar($id,$descripcion,$estado,$id_usuario_modifica,$fecha_modifica);
	...
	case 'desactivar':
		$rspta=$tipo_servicio_adicional->desactivar($id);
		echo $rspta ? "Datos desactivados correctamente" : "No se pudo desactivar los datos";
		break;
	case 'activar':
		$rspta=$tipo_servicio_adicional->activar($id);
		echo $rspta ? "Datos activados correctamente" : "No se pudo activar los datos";
		break;
	
	case 'mostrar':
		$rspta=$tipo_servicio_adicional->mostrar($id);
		echo json_encode($rspta);
		break;

    case 'listar':
		$rspta=$tipo_servicio_adicional->listar();
		$data=Array();

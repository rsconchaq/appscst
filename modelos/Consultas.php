...
public function comprasultimos_60dias(){
	$sql=" SELECT t.descripcion as tipoVehiculo,COUNT(s.id) as total from servicio s inner join tipo_vehiculo t on s.tipo_vehiculo_id=t.id 
	GROUP BY s.tipo_vehiculo_id,t.descripcion ORDER BY s.tipo_vehiculo_id,t.descripcion DESC LIMIT 0,60 ";
	return ejecutarConsulta($sql);
}

public function ventasultimos_12meses(){
	$sql="SELECT DATE_FORMAT(s.fecha_registro, '%W') AS fecha,COUNT(s.id) as total 
	from servicio s inner join tipo_vehiculo t on s.tipo_vehiculo_id=t.id 
	GROUP BY DATE_FORMAT(s.fecha_registro, '%W')
	ORDER BY s.fecha_registro DESC LIMIT 0,180";
	return ejecutarConsulta($sql);
}
